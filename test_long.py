...
if __name__ == "__main__":
    ...
    test_fpath_label = "/home/disk1/wangshaoying/my_video_retrieval/th14/data/test.txt"
    train_fpath_label = "/home/disk1/wangshaoying/my_video_retrieval/th14/data/val.txt"

    test_loader = load_data(long_root_folder, test_fpath_label, batch_size , False, 16, True)
    db_loader = load_data(long_root_folder, long_db_fpath_label, batch_size, False, 16, True)
    train_loader = load_data(long_root_folder, train_fpath_label, batch_size , False, 16, True)
    ...
    for epoch in range(int(num_epochs / 5)):
        backbone_path_dir = '/home/disk1/wangshaoying/video_src/video_gl/log/th14/backbone/' + f'{(epoch+1)*10}epoch.pth'
        hash_path_dir = '/home/disk1/wangshaoying/video_src/video_gl/log/th14/hash_layer/' + f'{(epoch+1)*10}epoch.pth'
        attention_path_dir = '/home/disk1/wangshaoying/video_src/video_gl/log/th14/temporal_attention/' + f'{(epoch+1)*10}epoch.pth'
        ...
        train_binary, train_labels = inference(train_loader, backbone, temporal_avg_pool, hash_layer, hash_length,
                                               device)
        test_binary, test_labels = inference(test_loader, backbone, temporal_avg_pool, hash_layer, hash_length, device)
        if attention:
            db_binary, db_labels = attention_inference(db_loader, backbone, temporal_attention, temporal_avg_pool,
                                                       hash_layer, hash_length, device)
        else:
